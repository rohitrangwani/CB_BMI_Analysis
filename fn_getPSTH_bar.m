...
for i = 1:size(TimeStamps1,1)
  for j = 2:size(TimeStamps1,2)
    ...
    if ~isempty(ts)
      ...
      PSTH1{i,j} = mean(psth); % returned by the function
...
%       psth_late  = smooth(mean(psth(floor(size(psth,1)/2)+1:end,:)),500);
      if enable_plot
%           psth_late  = mean(psth(floor(size(psth,1)/3)+1:end,:));
            psth_avg  = mean(psth);
%           bar(gca(h),bins(1:length(psth_avg)),psth_avg,'Facecolor','#77AC30');
%           bar(gca(h),bins(1:length(psth_avg)),psth_avg,'Facecolor','#D95319');
          bar(gca(h),bins(1:length(psth_avg)),psth_avg,'Facecolor','#0072BD');
          vline(before_zero,'k'); %xlim([2 6]); 
          title(['CH-',num2str(i),'-SC-',num2str(j)]);
          set(gca, 'TickDir', 'out')
    %       legend({'Early','Late'},'Location','northeast');
          saveas(gcf,[savepath,'PSTH-Ch_',num2str(i),'_Sc_',num2str(j),'_bar.fig']);clf;
      end
...
%       psth_early = smooth(mean(psth(1:floor(size(psth,1)/2),:)),500);
      psth_late  = smooth(mean(psth(floor(size(psth,1)/2)+1:end,:)),500);
%       plot(gca(h),bins(1:length(psth_early)),psth_early,'b','LineWidth',1.5); hold on;
      bar(gca(h),bins(1:length(psth_late)),psth_late,'m','LineWidth',1.5);
        vline(before_zero,'k'); %xlim([2 6]);
        title(['CH-',num2str(i),'-SC-',num2str(j)]);
        legend({'Early','Late'},'Location','northeast');
        set(gca, 'TickDir', 'out')
        if tetrodeSite == 0
          saveas(gcf,[savepath,'PSTH_Cb-Ch_',num2str(i),'_Sc_',num2str(j),'_bar.fig']);clf;
        else
          saveas(gcf,[savepath,'PSTH_M1-Ch_',num2str(i),'_Sc_',num2str(j),'_bar.fig']);clf;
        end
      end
    end
  end
