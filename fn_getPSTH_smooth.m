function [PSTH1,PSTH2,bins] = fn_getPSTH_smooth(TimeStamps1,events,Fs,savepath,before_zero,after_zero,tStart,tStop,TimeStamps2,meaOnlyFlag,tetrodeSite)
%-----------------------------------------------------------------------------------------------------------------------------
...
% Define valid events 
% events = events([tStart:tStart+75,end-5:end]); % just for custom plot
...
%       plot(gca(h),bins(1:length(psth_late)),psth_late,'m','LineWidth',1.5);
      psth_early = smooth(mean(psth(1:floor(size(psth,1)/3),:)),500);
      psth_late  = smooth(mean(psth(end-floor(size(psth,1)/3):end,:)),500);
      subplot(1,2,1)
      plot(gca(h),bins(1:length(psth_early)),psth_early,'b','LineWidth',4); hold on;
            subplot(1,2,2)
%       shadedErrorBar((1:length(psth(1:floor(size(psth,1)/2),:))),mean(psth(1:floor(size(psth,1)/2),:),1),std(psth(1:floor(size(psth,1)/2),:)),'lineProps','b');
      plot(gca(h),bins(1:length(psth_late)),psth_late,'m','LineWidth',4);
% shadedErrorBar((1:length(psth(floor(size(psth,1)/2)+1:end,:))),mean(psth(floor(size(psth,1)/2)+1:end,:),1),std(psth(floor(size(psth,1)/2)+1:end,:)),'lineProps','m');      vline(before_zero,'k'); %xlim([2 6]); 
      title(['CH-',num2str(i),'-SC-',num2str(j)]);
      legend({'Early','Late'},'Location','northeast');
      saveas(gcf,[savepath,'PSTH-Ch_',num2str(i),'_Sc_',num2str(j),'_smooth.fig']);clf;
%       if meaOnlyFlag == 1 % 1-16 Ch MEA in Cb 17-32 Ch MEA in M1
%         if i<=16
%           saveas(gcf,[savepath,'PSTH_Cb-Ch_',num2str(i),'_Sc_',num2str(j),'_smooth.tiff']);clf;
%         else
%           saveas(gcf,[savepath,'PSTH_M1-Ch_',num2str(i),'_Sc_',num2str(j),'_smooth.tiff']);clf;
%         end
  ...
  for i = 1:size(TimeStamps2,1)
    for j = 2:size(TimeStamps2,2)
      ...
      if ~isempty(ts)
        ...
        % for plotting and saving only
      psth_early = smooth(mean(psth(1:floor(size(psth,1)/2),:)),500);
      psth_late  = smooth(mean(psth(floor(size(psth,1)/2)+1:end,:)),500);
      plot(gca(h),bins(1:length(psth_early)),psth_early,'b','LineWidth',1.5); hold on;
      plot(gca(h),bins(1:length(psth_late)),psth_late,'m','LineWidth',1.5);
