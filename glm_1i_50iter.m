% GLM model - for M1 predicting CB indirect - GLM-1i
...
TU = 0; % flag for task-unrelated units, not used for this analysis
...
%         set(gcf,'WindowState','maximized')
        ...
        saveas(gcf,[rootpath,'\',sub{s},'\','M1CbUnits_',num2str(j),'.fig']);
        close;
        all_bins{k}=ii;
%     end
    figure;
...
%     histogram(cell2mat(all_bins(:)));
    bar(GR,GC/1.0);
    saveas(gcf,[rootpath,'\',sub{s},'\','M1_Cb_hist_',num2str(j),'.fig']);
    close;
    end
...
for s=M1_stroke
    ...
    load([rootpath, sub{s}, '\GLM_M1_Cb.mat'],'all_R2');
%     col = colormap(size(all_R2,3));
    for j=1:10
        if (sig_session_bin{s}(j))
        avg = mean((all_R2),2);
        stderr = std(all_R2,[],2);
        stderr = stderr./sqrt(size(all_R2,2)-1);
        r2{count} = avg(:,:,j);
        tmp = cell2mat(r2(count));
        if(sum(tmp))
            h = plot(tmp(2:4),'.-','MarkerSize',15,'LineWidth',2); hold on;
            e = errorbar([1,2,3],tmp(2:4),stderr(2:4),'.','LineWidth',1);
            e.Marker = 'none';
            e.Color = get(h,'Color');
            count = count +1;
        end
...
% r2_all = cell2mat(r2);
% r2_avg = mean(r2_all,2);
% plot(r2_avg(2:4),'.-','MarkerSize',20,'LineWidth',4,'Color','k'); hold on;
yline(0);
