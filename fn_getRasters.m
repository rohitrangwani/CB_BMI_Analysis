...
% Define valid events 
% events = events([tStart:30,60:65,95:tStop]); % just for custom plot
...
%           scatter(gca(h),raster,n*ones(1,length(raster)),40,'|','markeredgecolor','#77AC30','linewidth',1.4);hold on;
...
%           scatter(gca(h),raster,n*ones(1,length(raster)),30,[140 140 140]/255,'filled');hold on;
      ...
      if meaOnlyFlag == 1 % 1-16 Ch MEA in Cb 17-32 Ch MEA in M1
        if i<=16
          saveas(gcf,[savepath,'RASTERS_Cb-Ch_',num2str(i),'_Sc_',num2str(j),'.fig']);clf;
        else
          saveas(gcf,[savepath,'RASTERS_Cb-Ch_',num2str(i),'_Sc_',num2str(j),'.fig']);clf;
        end
