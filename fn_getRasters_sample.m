function fn_getRasters_sample(TimeStamps1,events,revents,Fs,savepath,before_zero,after_zero,tStart,tStop,TimeStamps2,meaOnlyFlag,tetrodeSite)
%----------------------------------------------------------------------------------------------------------------
...
for i = 1:size(TimeStamps1,1)
  for j = 2:size(TimeStamps1,2)
    ts = TimeStamps1{i,j};
    ts = ts(1:3:end); % taking every third sample
%     [N,ts] = histcounts(TimeStamps1{i,j},size(TimeStamps1{i,j})/5);
...
%           scatter(gca(h),raster,n*ones(1,length(raster)),30,[140 140 140]/255,'filled');hold on;
          scatter(gca(h),raster,n*ones(1,length(raster)),40,'s','k','filled');hold on;
        end
        end
      end
...
%         if i<=16
%           saveas(gcf,[savepath,'RASTERS_Cb-Ch_',num2str(i),'_Sc_',num2str(j),'_sample.tiff']);clf;
%         else
%           saveas(gcf,[savepath,'RASTERS_M1-Ch_',num2str(i),'_Sc_',num2str(j),'_sample.tiff']);clf;
%         end
%       else % 32 Ch MEA only in M1
        saveas(gcf,[savepath,'RASTERS-Ch_',num2str(i),'_Sc_',num2str(j),'_sample.tiff']);clf;
%       end
    end
  end
end
