...
freq_band = {[0.1 4] [3 6] [6 14] [13 30] [30 50] [0.1 50] };
...
robust_session = {[4,5],[3,4,6],[5,7,8,9,10,13,14],[3,4,5,8,9,10],[5,6,7,9]...
    [4,6,8],[5:9],[8,10],[7:9],[3,5,6],[8,14,15,16,18,19]};
...
display(meT_M1)
    ...
    saveas(gcf,[rootpath,'\..\AllTrial_BandPower_M1_',num2str(f),'.tiff']);
    close;
...
%     plot([1,meT_Cb(:)],[2,mlT_Cb(:)]);
    ...
    save([rootpath,'\..\AllTrial_LFP_power_consolidated',num2str(f),'.mat'],'meT_M1','mlT_M1',...
          'meT_Cb','mlT_Cb');
    
    set(gcf,'WindowState','maximized')
    saveas(gcf,[rootpath,'\..\Alltrial_BandPower_Cb_',num2str(f),'.tiff']);
    close;
end
end
end
...
%         load([path, sub{s},'\LFP_power_consolidated',num2str(i),'.mat']);
        load([path, sub{s},'\Alltrial_LFP_power_consolidated',num2str(i),'.mat']);
      else
%          load([path, sub{s},'\Data\','\LFP_power_consolidated',num2str(i),'.mat']);
         load([path, sub{s},'\Data\','\Alltrial_LFP_power_consolidated',num2str(i),'.mat']); 
      end
      meT_all{s,i} = meT_Cb;
      mlT_all{s,i} = mlT_Cb;
      end
