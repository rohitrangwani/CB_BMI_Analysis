%% Generate training vids for DLC
...
robust_session = {[4,5],[3,4,6],[5,7,8,9,10,13,14],[3,4,5,8,9,10],[5,6,7,9],...
    [4,6,8],[5:9],[8],[7:8],[3,5,6],[8,14,15,16,18,19],[7,9,11,14],[8,9,10,12],[5,9,10]};    
...
for s=14
    ...
    for n=robust_session{s}(rand_idx)
        ...
        savedir = [rootpath, '\Train_vids\', bmiBlocks(n).name,'\Trial_vid\'];
        ...
        for tr=1:length(random_trials)
            clear frames vW
            frames = read(vR,[round((all_trials(random_trials(tr))/Fs+0.5)*vR.FrameRate),round((all_trials(random_trials(tr))/Fs+16)*vR.FrameRate)]);
...
%         random_trials  = randi([tstart{s}(n),tstop{s}(n)],1,2);
        ...
        for tr=tstart{s}(n):tstop{s}(n)
            clear frames vW
                frames = read(vR,[round((all_trials((tr))/Fs+0.5)*vR.FrameRate),round((all_trials((tr))/Fs+16)*vR.FrameRate)]);
%                 frames = read(vR,[round((rewards_onset((tr))/Fs-2.2)*vR.FrameRate),round((rewards_onset((tr))/Fs+0.2)*vR.FrameRate)]);
                vW = VideoWriter([savedir,bmiBlocks(n).name,'_Tr_',num2str(tr),'.avi'],'Motion JPEG AVI');
                vW.FrameRate = vR.FrameRate;
                open(vW);
                writeVideo(vW,frames);
                close(vW);
